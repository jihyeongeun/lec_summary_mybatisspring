...
//        List<Integer> menuCode = orderInfo.getOrderMenus().stream().map(OrderMenuDTO::getMenuCode).collect(Collectors.toList());
        ...
        List<OrderMenuDTO> orderMenus = orderInfo.getOrderMenus();     // 사용자가 입력한 list를 뽑아서 menuCode라는 곳에 하나씩 저장
...
//        System.out.println("menuCode = " + menuCode);
        ...
        /* 설명 2. 주문한 메뉴 별로 Menu 엔티티에 담아서 조회(select)해오기(부가적인 메뉴의 정보 추출) */
        ...
        /* 설명 4. 1부터 3까지를 하면 tbl_order 테이블에 추가(insert)이 가능하다. */
        /* 설명 4-1. insert를 하기 위해 테이블과 매칭되는 Entity로 옮겨 담는다.(DTO -> Entity)(dto를 entity로 바꾸는 작업) */
...
//        );
        ...
        for (int i = 0; i < orderMenuSize; i++) {
            ...
            orderMenu.setOrderCode(order.getOrderCode());        // entity는 setter가 없었으므로 추가함
